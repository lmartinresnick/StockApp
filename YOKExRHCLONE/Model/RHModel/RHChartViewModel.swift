...
class RHChartViewModel: ObservableObject {
    ...
    init(symbol: String) {
        ...
        let assignees: [ReferenceWritableKeyPath<RHChartViewModel, PlotData?>] = [
            \.intradayPlotData,
            \.dailyPlotData,
            \.weeklyPlotData,
            \.monthlyPlotData
    ...
    static func segmentByDays(values: PlotData) -> [Int] {
        ...
        let lastStopper = calendar.startOfWeekOfYear(for: values.last!.time)
        
        // Work backward from last week
        let breakpoints = (0..<values.count).map {
    ...
    private func mapToPlotData(_ response: RHChartAPIResponse?) -> PlotData? {
